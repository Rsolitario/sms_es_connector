...
class TestQueueLogic(TransactionCase):
    ...
    def test_01_deduplication_logic(self):
        ...
        self.SmsMessage.create({
            'name': 'Original SMS', 'sender': 'TestSender', 'receiver': '111222333',
            'text': 'This is a test message.', 'state': 'delivered'
        })

        # Intentar crear y encolar un mensaje idéntico
        duplicate_msg = self.SmsMessage.create({
            'name': 'Duplicate SMS', 'sender': 'TestSender', 'receiver': '111222333',
            'text': 'This is a test message.', 'state': 'draft'
        })
        ...
        job_count = self.QueueJob.search_count([('message_id', '=', duplicate_msg.id)])
        self.assertEqual(job_count, 0)

    @patch('odoo.addons.sms_es_connector.models.sms_es_client.SmsEsClient.send_sms')
    def test_02_worker_backoff_on_failure(self, mock_send_sms):
        """Prueba que el worker programe un reintento si el envío falla."""
        mock_send_sms.return_value = {'status': 'failed', 'error': {'code': 503, 'message': 'Service Unavailable'}}

        msg = self.SmsMessage.create({
            'name': 'Test Backoff', 'sender': 'Odoo', 'receiver': '444555666',
            'text': 'Test retry logic.', 'state': 'draft'
        })
        ...
        self.assertEqual(job.state, 'pending') # Vuelve a pendiente para el reintento
