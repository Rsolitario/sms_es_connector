...
from odoo.addons.sms_es_connector.models.sms_es_client import (
    SmsEsClient,
    RC_THROTTLING_ERROR,
)
...
class TestSmsEsApiClient(BaseCase):
    ...
    def setUp(self):
        ...
        self.env["ir.config_parameter"].set_param(
            "sms_es_connector.api_url", "http://fakeapi.com/sendsms"
        )
        self.env["ir.config_parameter"].set_param(
            "sms_es_connector.api_username", "user"
        )
        self.env["ir.config_parameter"].set_param(
            "sms_es_connector.api_password", "pass"
        )
    ...
    def test_02_send_sms_rejected_420(self, mock_post):
        ...
        mock_response.json.return_value = {
            "error": {"code": 101, "message": "Authentication failed"}
        }
        mock_post.return_value = mock_response
    ...
    def test_03_send_sms_throttling_retry_105(self, mock_post, mock_sleep):
        ...
        success_response = MagicMock(
            status_code=202, json=lambda: {"msgId": "uuid", "numParts": 1}
        )
        throttling_response = MagicMock(
            status_code=420,
            json=lambda: {
                "error": {"code": RC_THROTTLING_ERROR, "message": "Throttling"}
            },
        )
