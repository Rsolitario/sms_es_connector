...
class ResConfigSettings(models.TransientModel):
    ...
    sms_es_dlr_url = fields.Char(
        ...
        help="URL que debe configurar en su proveedor de SMS para recibir los DLRs."
    )
    ...
    sms_es_use_flash = fields.Boolean(
        ...
        help="Si se activa, los mensajes se enviarán como 'flash' (no se guardan en el dispositivo)."
    )
    sms_es_use_validate_period = fields.Boolean(
        ...
        help="Permite definir un tiempo máximo de validez para el mensaje en la red."
    )
    sms_es_validate_period_minutes = fields.Integer(
        ...
        help="Tiempo en minutos durante el cual el SMS será válido para su entrega."
    )
    ...
    sms_es_cron_frequency_minutes = fields.Integer(
        ...
        default=1,
        help="Intervalo en minutos con el que el cron procesará la cola de envíos."
    )
    ...
    sms_es_webhook_token = fields.Char(
        ...
        help="Token secreto para añadir a la URL del webhook como medida de seguridad."
    )
    ...
    def get_values(self):
        ...
        if cron:
            res.update(
                sms_es_cron_frequency_minutes=cron.interval_number
            )
        return res
    ...
    @api.depends('sms_es_api_url', 'sms_es_webhook_token') # Añadir dependencia del token
    def _compute_dlr_url(self):
        """Calcula la URL del webhook que el usuario debe configurar en el proveedor."""
        config_params = self.env['ir.config_parameter'].sudo()
        base_url = config_params.get_param('web.base.url')
        token = self.sms_es_webhook_token or config_params.get_param('sms_es_connector.webhook_token')
        
        for settings in self:
            if base_url and token:
                settings.sms_es_dlr_url = f"{base_url}/sms_es_connector/webhook/dlr?token={token}"
            elif not base_url:
                settings.sms_es_dlr_url = "La 'web.base.url' del sistema no está configurada."
            else:
                settings.sms_es_dlr_url = "Guarde la configuración para generar el token y la URL completa."