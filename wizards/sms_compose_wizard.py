...
class SmsComposeWizard(models.TransientModel):
    ...
    res_ids_str = fields.Char('IDs de Origen', readonly=True) # Usamos un string para evitar problemas de tamaño
    ...
    ], string="Codificación", required=True)
    ...
    use_validate_period = fields.Boolean(string="Establecer Periodo de Validez")
    ...
    def default_get(self, fields_list):
        ...
        if self.env.context.get('active_ids'):
            res['res_ids_str'] = ','.join(map(str, self.env.context['active_ids']))
    ...
    def _get_recipient_number(self, record):
        ...
        if self.res_model == 'crm.lead':
            ...
            if record.phone:
                return record.phone
            # Prioridad 2: Si no tiene, buscar en el contacto asociado (si existe)
            if record.partner_id:
                return record.partner_id.mobile or record.partner_id.phone
            return None # No se encontró número

        # 3. Lógica genérica para otros modelos (Ventas, Facturas) que usan partner_id
        if hasattr(record, 'partner_id'):
            # Usamos getattr para acceder de forma segura a 'partner_shipping_id',
            # que puede no existir. Si no existe, usamos partner_id.
            partner = getattr(record, 'partner_shipping_id', None) or record.partner_id
            if partner:
    ...
    def action_send_sms(self):
        ...
        if not all_messages:
             raise UserError(f"No se pudo crear ningún mensaje. Verifique que los siguientes registros tengan un número de teléfono o móvil:\n\n" + "\n".join(skipped_records))
        ...
        if skipped_records:
            ...
            _logger.warning("No se pudo enviar SMS a los siguientes registros por falta de número: %s", ", ".join(skipped_records))
