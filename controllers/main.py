...
class SmsEsWebhookController(http.Controller):
    ...
    @http.route('/sms_es_connector/webhook/dlr', type='http', auth='none', methods=['POST'], csrf=False, save_session=False)
    def handle_dlr_webhook(self, **kwargs):
        """
        Controlador para recibir y procesar los informes de entrega (DLR).
        """
        _logger.info("Recibido DLR de SMS.es. Headers: %s, Body: %s", request.httprequest.headers, request.httprequest.data)
        ...
        if not received_token or not hmac.compare_digest(received_token, expected_token or ''):
            _logger.warning("Intento de acceso al webhook DLR con token inválido: %s", received_token)
        ...
        if hmac_secret:
            ...
            if not signature:
                _logger.warning("Petición DLR recibida sin firma HMAC, pero hay un secreto configurado.")
                return request.make_response("Forbidden: Missing Signature", status=403)
            ...
            if not hmac.compare_digest(digest, signature):
                ...
                return request.make_response("Forbidden: Invalid Signature", status=403)
        ...
        try:
            ...
            return request.make_response("Bad Request: Malformed JSON", status=400)
        ...
        if odoo_message_id:
            try:
                message = request.env['sms_es.message'].sudo().browse(int(odoo_message_id))
                if not message.exists():
                    message = request.env['sms_es.message'].sudo().browse() # Reset if not found
            except (ValueError, TypeError):
                 _logger.warning("odoo_message_id '%s' no es un entero válido.", odoo_message_id)

        # Prioridad 2 (Fallback): Buscar por msgId
        if not message and msg_id:
            message = request.env['sms_es.message'].sudo().search([('msg_id', '=', msg_id)], limit=1)

        if not message:
            _logger.warning("No se encontró un mensaje en Odoo para el DLR recibido. odoo_id: %s, msg_id: %s", 
                            odoo_message_id, msg_id)
            # Devolvemos 200 para que el proveedor no reintente. El DLR es válido.
        ...
        try:
            ...
            if new_state:
                ...
                _logger.info("Mensaje ID %d actualizado al estado '%s' por el evento DLR '%s'.", message.id, new_state, event)
            ...
            dlr_vals = {
                ...
                'custom': json.dumps(dlr_data.get('custom')) if dlr_data.get('custom') else False,
            }
            request.env['sms_es.dlr_event'].sudo().create(dlr_vals)
            _logger.info("Evento DLR registrado para el mensaje ID %d.", message.id)

            # Confirmar la transacción
            request.env.cr.commit()

        except Exception as e:
            _logger.error("Error al procesar y guardar el DLR para el mensaje ID %d: %s", message.id if message else 0, e)
